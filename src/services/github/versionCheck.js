class VersionCheck {
  ...
  async getVersion() {
    ...
    try {
      ...
      this.logger.info(`Latest version is ${data.tag_name} and current version is ${this.currentVersionStr}`)

      const latestVersion = data.tag_name;
      if (parseInt(latestVersion.slice(1).split('.').join('')) > parseInt(this.currentVersion.split('.').join(''))) {
